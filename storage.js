const DB_KEY="tidrapport_pro_v7";
function emptyDb(){return{profile:{},companies:[{id:"default-company",name:"Diga.nu",org:"",email:"",phone:""}],projects:[{id:"default-project",number:"P1012",name:"Standardprojekt"}],customers:[],workplaces:[],reports:[],drives:[],settings:{theme:"light"},timer:{running:false,start:0,elapsed:0}}}
...
function updateReport(reportId,data){const db=loadDb();const i=db.reports.findIndex(r=>r.id===reportId);if(i<0)return{success:false,errors:["Rapporten hittades inte"]};const merged={...db.reports[i],...data,hours:num(data.hours),hourRate:num(data.hourRate),km:num(data.km),kmRate:num(data.kmRate),parking:num(data.parking),toll:num(data.toll),material:num(data.material),breakMinutes:num(data.breakMinutes),updatedAt:new Date().toISOString()};const v=validateReport(merged);if(!v.valid)return{success:false,errors:v.errors};db.reports[i]=merged;saveDb(db);return{success:true,report:merged}}
...
function projectTotals(){const map={};getReports().forEach(r=>{const p=byId("projects",r.projectId);const name=p?`${p.number||""} ${p.name||""}`.trim():"Projekt saknas";if(!map[name])map[name]={hours:0,cost:0,pay:0};map[name].hours+=num(r.hours);map[name].cost+=reportCost(r);map[name].pay+=reportPay(r)});return map}
