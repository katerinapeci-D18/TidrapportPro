const DB_KEY="timereport_pro_v8";
function emptyDb(){return{setup:false,profile:{},companies:[],projects:[{id:"default-project",number:"P1012",name:"Standard project"}],customers:[],workplaces:[],reports:[],drives:[],settings:{theme:"light",language:"sv",currency:"SEK",country:"Sweden"},timer:{running:false,start:0,elapsed:0}}}
...
function currency(){return loadDb().settings.currency||"SEK"}
function money(v){return `${Math.round(num(v))} ${currency()}`}
...
function validateReport(r){const e=[];if(!r.date)e.push("Date missing");if(num(r.hours)<=0)e.push("Hours must be more than 0");return{valid:e.length===0,errors:e}}
function addReport(data){const v=validateReport(data);if(!v.valid)return{success:false,errors:v.errors};const db=loadDb();const report={id:id(),date:data.date,companyId:data.companyId,projectId:data.projectId,customerId:data.customerId,workplaceId:data.workplaceId,gps:data.gps||"",startTime:data.startTime||"",endTime:data.endTime||"",breakMinutes:num(data.breakMinutes),hours:num(data.hours),hourRate:num(data.hourRate),km:num(data.km),kmRate:num(data.kmRate),parking:num(data.parking),toll:num(data.toll),material:num(data.material),comment:(data.comment||"").trim(),attachments:data.attachments||[],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};db.reports.push(report);db.last={companyId:report.companyId,projectId:report.projectId,customerId:report.customerId,workplaceId:report.workplaceId};saveDb(db);return{success:true,report}}
function updateReport(reportId,data){const db=loadDb();const i=db.reports.findIndex(r=>r.id===reportId);if(i<0)return{success:false,errors:["Report not found"]};const merged={...db.reports[i],...data,hours:num(data.hours),hourRate:num(data.hourRate),km:num(data.km),kmRate:num(data.kmRate),parking:num(data.parking),toll:num(data.toll),material:num(data.material),breakMinutes:num(data.breakMinutes),updatedAt:new Date().toISOString()};const v=validateReport(merged);if(!v.valid)return{success:false,errors:v.errors};db.reports[i]=merged;saveDb(db);return{success:true,report:merged}}
...
function reportPay(r){return num(r.hours)*num(r.hourRate)+num(r.km)*num(r.kmRate)+reportCost(r)}
function todayReports(){const d=new Date().toISOString().slice(0,10);return getReports().filter(r=>r.date===d)}
...
function projectTotals(){const map={};getReports().forEach(r=>{const p=byId("projects",r.projectId);const name=p?`${p.number||""} ${p.name||""}`.trim():"No project";if(!map[name])map[name]={hours:0,cost:0,pay:0};map[name].hours+=num(r.hours);map[name].cost+=reportCost(r);map[name].pay+=reportPay(r)});return map}
